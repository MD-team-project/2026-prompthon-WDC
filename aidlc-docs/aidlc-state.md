...
- [ ] FE: Functional Design -> Code Generation
  ...
  **FE post-summary iteration** 2026-08-20T21:45:00Z, docs corrected to match the code:
  - **Real character art arrived for `massagechair`**: 121 `.webp` frames, 2.3 MB, `levelup/` 56 and `surprise/` 65 at 40ms, served static not bundled. `pral` and `shoecase` keep the CSS placeholder. **BE/INFRA note**: static asset serving is now a real deployment concern, not a placeholder one
